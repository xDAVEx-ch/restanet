export const FourSquareAPIService = (() => {
  ...
  const controller = {
    call (parameters, newEndpoint, idParameter) {
      const url = this.buildUrl(newEndpoint, idParameter)

      return new Promise((resolve, reject) => {
        window.fetch(`${url}&${parameters[0]}&${parameters[1]}`, {
          method: 'GET'
        }).then((response) => {
    ...
    buildUrl (newEndpoint, idParameter) {
      ...
      const venueIdParameter = idParameter

      const url = `https://api.foursquare.com/v2/venues${'/' + venueIdParameter}/${endpoint}?client_id=${clientId}&client_secret=${clientSecret}&v=20190425&limit=3`
